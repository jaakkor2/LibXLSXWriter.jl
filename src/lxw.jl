...
lxw_chart_data_label(; value="", hide=0x0, font=C_NULL, line=C_NULL, fill=C_NULL, pattern=C_NULL) =
...
lxw_chart_point(; line=C_NULL, fill=C_NULL, pattern=C_NULL) =
   Ref(lxw_chart_point(
      Base.unsafe_convert(Ptr{lxw_chart_line}, line),
      Base.unsafe_convert(Ptr{lxw_chart_fill}, fill),
      Base.unsafe_convert(Ptr{lxw_chart_pattern}, pattern)
...
lxw_table_column(; header=C_NULL, formula=C_NULL, total_string=C_NULL, total_function=false,
   header_format=C_NULL, format=C_NULL, total_value=0.0) =
   Ref(lxw_table_column(
      Cs(header),
      Cs(formula),
      Cs(total_string),
      total_function,
...
lxw_table_options(; name=C_NULL, no_header_row=false, no_autofilter=false, no_banded_rows=false,
   banded_columns=false, first_column=false, last_column=false,
   style_type=LXW_TABLE_STYLE_TYPE_MEDIUM, style_type_number=9, total_row=false, columns=C_NULL) =
   Ref(lxw_table_options(
      Cs(name),
      no_header_row,
      no_autofilter,
      no_banded_rows,
