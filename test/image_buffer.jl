#=
https://github.com/jmcnamara/libxlsxwriter/blob/main/examples/image_buffer.c
/*
 * An example of inserting an image from a memory buffer into a worksheet
 * using the libxlsxwriter library.
 *
 * Copyright 2014-2025 John McNamara, jmcnamara@cpan.org
 *
 */
=#

using LibXLSXWriter: workbook_new, workbook_add_worksheet, lxw_image_options, worksheet_insert_image_buffer, worksheet_insert_image_buffer_opt, workbook_close

# Simple array with some PNG data.
image_buffer = [
    
]

image_size = 200


# Create a new workbook and add a worksheet.
workbook = workbook_new("image_buffer.xlsx")
worksheet = workbook_add_worksheet(workbook, C_NULL)

options = lxw_image_options(x_offset = 34, y_offset = 4, x_scale = 2, y_scale = 1)

# Insert the image from the buffer.
worksheet_insert_image_buffer(worksheet, CELL("B3"), image_buffer, image_size)

# Insert the image from the same buffer, with some options.
worksheet_insert_image_buffer_opt(worksheet, CELL("B7"), image_buffer, image_size, options)

workbook_close(workbook)
